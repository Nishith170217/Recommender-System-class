...
def perarsonSimilarity(list1,list2):
    ...
    cross_sum = 0.
    for i in range(M):
        var_sum1 += (list1[i] - mean1) ** 2
        var_sum2 += (list2[i] - mean2) ** 2
        cross_sum += (list1[i] * list2[i])
...
for i in range(len(ratingValues) - 1):
    for j in range(i + 1, len(ratingValues)):
        userSimMatrix[i, j] = perarsonSimilarity(ratingValues[i], ratingValues[j])
